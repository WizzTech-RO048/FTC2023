...
public class StatePositionRight extends State {
    private boolean hasMovedForward = false;
    private boolean hasMovedLeft = false;

    public StatePositionRight(@NonNull Robot robot) {
        super(robot);
    }
    ...
    public State update() {
        ...
        if (!hasMovedLeft) {
            hasMovedLeft = true;
            return new StateMove(robot, 0.5, 0.3, Wheels.MoveDirection.RIGHT, this, 10);
        }
